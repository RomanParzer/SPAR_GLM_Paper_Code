...
spar_res_darwin_big <- spar.cv(darwin_big$x,darwin_big$y,family=binomial(logit),
...
plot(spar_res_darwin,"coefs",coef_order = reorder_ind) +
  geom_vline(xintercept = 0.5 + 1:17*25,linetype=2,linewidth=0.4) +
  annotate("text",x=0:17*25 + 12,y=35,label=feat_names,angle=90,size=3) +
  labs(fill="coef value",y="index of marginal model")
# ggsave(paste0("../plots/spar_coef_darwin.pdf"), height = 3, width = 8)

# colnames(darwin$x)[reorder_ind]


plot(spar_res_darwin_big)
plot(spar_res_darwin_big,"coefs") +
  geom_vline(xintercept=ncol(darwin$x)+0.5,alpha=0.2,linetype=2) +
  labs(fill="coef value") +
  scale_fill_gradient2(transform = scales::transform_modulus(p=1/10))
# ggsave(paste0("../plots/spar_coef_darwin_big_mod.pdf"), height = 4, width = 8)

plot(spar_res_darwin_big,"coefs") +
  geom_vline(xintercept=ncol(darwin$x)+0.5,alpha=0.2,linetype=2) +
  labs(fill="coef value") 
# ggsave(paste0("../plots/spar_coef_darwin_big.pdf"), height = 4, width = 8)

data.frame(predictor=1:ncol(darwin_big$x),sum_abs_coef_value=apply(abs(spar_res_darwin_big$betas),1,sum)) %>%
  ggplot(aes(x=predictor,y=sum_abs_coef_value)) +
  geom_point() +
  geom_vline(xintercept=ncol(darwin$x)+0.5,alpha=0.2,linetype=2) +
  labs(y="sum of absolute coefficients")
# ggsave(paste0("../plots/spar_coef_darwin_big_abs.pdf"), height = 4, width = 8)

# how many of estimated top variables are in the first half; 
# around 51 %, not super convincing, but prob other methods not better
darwin_big_coef <- coef(spar_res_darwin_big)
mean(order(abs(darwin_big_coef$beta),decreasing = TRUE)[1:ncol(darwin$x)]<=ncol(darwin$x))
...
# how many of estimated top variables are in the first half; 
# around 82 %, not super convincing, but prob other methods not better
