...
getAnywhere(screening_ridge_lambda0())
################ adapt #########################################################################
...
parres <- foreach(j = 35) %:%
  foreach(i=1:nrep) %dopar% {
    ...
    # k <- 15
    # for (k in 1:nmethods) {
    tmp_methods <- c(4,8,9,10,11,12,13,14,15,16,18)
    for (k in tmp_methods)  {
      set.seed((1234+i)^2 + k)
      tstamp <- Sys.time()
...
apply(res[,,tmp_methods,1],c(2,3),mean)

summary(callres$scr_coef)
