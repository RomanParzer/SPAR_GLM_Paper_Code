...
simulation_settings <- tibble(n=200, p=c(2000),ntest=200, act_setting=c("sparse","medium","dense"), cov_setting=c("group"),
                              signal_strength=1/4, avg_exp = 10,family=list(poisson(log)))
...
my.cluster <- parallel::makeCluster(3, type = "PSOCK", outfile = "../saved_results/log.txt")
...
# saveRDS(res,"../saved_results/Result_RP_Experiment_poi.rds")

res <- readRDS("../saved_results/Result_RP_Experiment_poi.rds")
...
tmp_df <- mydf_all %>% filter(MSPE<50,MSLE<2) %>%
...
# ggsave("../plots/RPexperiment_poi_all.pdf", height = 5, width = 8)
...
# ggsave("../plots/ScrExperiment_poi_all.pdf", height = 5, width = 8)
