...
simulation_settings <- tibble(n=200, p=c(2000),ntest=1000, act_setting="medium", cov_setting=c("group"),
                              signal_strength=c(10,1/8,100,1,5,1000,1/4), avg_exp = c(1,10,0.5,0.5,0.5,0.7,10),
                              family=list(gaussian(identity),gaussian(log),binomial(logit),binomial(logit),mybinom_fam,binomial(cloglog),poisson(log)),
                              weight_binom = c(1,1,1,100,1,1,1),m=n/4)

# different m
simulation_settings <- rbind(simulation_settings,
                             tibble(n=200, p=c(2000),ntest=1000, act_setting="medium", cov_setting=c("group"),
                                    signal_strength=c(5,1/16,50,0.5,2.5,500,1/8), avg_exp = c(1,10,0.5,0.5,0.5,0.7,10),
                                    family=list(gaussian(identity),gaussian(log),binomial(logit),binomial(logit),mybinom_fam,binomial(cloglog),poisson(log)),
                                    weight_binom = c(1,1,1,100,1,1,1),m=n/2))

simulation_settings <- rbind(simulation_settings,
                             tibble(n=200, p=c(2000),ntest=1000, act_setting="medium", cov_setting=c("group"),
                                    signal_strength=c(5,1/16,50,0.5,2.5,500,1/8), avg_exp = c(1,10,0.5,0.5,0.5,0.7,10),
                                    family=list(gaussian(identity),gaussian(log),binomial(logit),binomial(logit),mybinom_fam,binomial(cloglog),poisson(log)),
                                    weight_binom = c(1,1,1,100,1,1,1),m=3*n/4))

simulation_settings <- rbind(simulation_settings,
                             tibble(n=200, p=c(2000),ntest=1000, act_setting="medium", cov_setting=c("group"),
                                    signal_strength=c(5,1/16,50,0.5,2.5,500,1/8), avg_exp = c(1,10,0.5,0.5,0.5,0.7,10),
                                    family=list(gaussian(identity),gaussian(log),binomial(logit),binomial(logit),mybinom_fam,binomial(cloglog),poisson(log)),
                                    weight_binom = c(1,1,1,100,1,1,1),m=round(3*log(p))))

simulation_settings <- rbind(simulation_settings,
                             tibble(n=200, p=c(2000),ntest=1000, act_setting="medium", cov_setting=c("group"),
                                    signal_strength=c(5,1/16,50,0.5,2.5,500,1/8), avg_exp = c(1,10,0.5,0.5,0.5,0.7,10),
                                    family=list(gaussian(identity),gaussian(log),binomial(logit),binomial(logit),mybinom_fam,binomial(cloglog),poisson(log)),
                                    weight_binom = c(1,1,1,100,1,1,1),m=round(log(p))))

# same with less signal_strength
simulation_settings <- rbind(simulation_settings,
                             tibble(n=200, p=c(2000),ntest=1000, act_setting="medium", cov_setting=c("group"),
                                    signal_strength=c(5,1/16,50,0.5,2.5,500,1/8), avg_exp = c(1,10,0.5,0.5,0.5,0.7,10),
                                    family=list(gaussian(identity),gaussian(log),binomial(logit),binomial(logit),mybinom_fam,binomial(cloglog),poisson(log)),
...
methods <- c("ElNet","AdLASSO","Gaussian","Sparse","SparseCW",
             "GLM_HOLP","L2_cv",
             "L2_sqrtnp","L2_10","L2_1","L2_01","L2_001",
             "L2_dev06","L2_dev08","L2_dev095","L2_dev0999",
             "marGLM","True_Beta")
measures <- c("AUC","MSPE","MSLE","pAUC","Cor_scr","pAUC_scr","lambda","dev.ratio","mtr_over_mf_abs","TrRatio_3a","check_gy")
...
foreach(j = 1:nset) %:%
  ...
  c(n,p,ntest,act_setting,cov_setting,signal_strength,avg_exp,family,weight_binom,m,a) %<-% simulation_settings[j,]
  family <- family[[1]]
  set.seed((1234+i)^2)
  data <- generate_data_glm(n,p,family=family,cov_setting=cov_setting,ntest=ntest,
                            a=a,signal_strength = signal_strength, avg_exp = avg_exp,weight_binom=weight_binom)
  ...
  check_gy <- !is.nan(sum(family$linkfun(y))*0)
  ...
  if (family$family=="binomial" & weight_binom==1) {
    ...
    tmp <- as.numeric(crossprod(z,myinv%*%(family$linkfun(y_corr))))
    mean(abs(coefs[,1]-tmp))
  }
  resij[c(7,8),6] <- c(0,1)
  ...
  glmnet_res <- glmnet::cv.glmnet(x=z,y=y_fit,family = family,alpha=0)
  coefs[,2] <- coef(glmnet_res)[-1]
  resij[c(7,8),7] <- c(glmnet_res$lambda.1se,glmnet_res$glmnet.fit$dev.ratio[glmnet_res$index["1se",1]])
  
  glmnet_res <- glmnet::glmnet(x=z,y=y_fit,family = family,alpha=0,lambda = c(sqrt(n)+sqrt(p),10,1,0.1,0.01))
  coefs[,3:7] <- as.matrix(glmnet_res$beta)
  resij[c(7,8),8:12] <- rbind(glmnet_res$lambda,glmnet_res$dev.ratio)
  
  glmnet_res <- glmnet::glmnet(x=z,y=y_fit,family = family,alpha=0,lambda.min.ratio = 1e-4 /lam_max)
  for (l in 1:4) {
    mydevrat <- c(0.6,0.8,0.95,0.999)[l]
  ...
  #   apply(coefs,2,function(col2) mean((col1-col2)^2))
  # })
  # # mseMat
  ...
  resij[-c(7,8),3:5] <- sapply(reductions, function(Phi){
    ...
    glmnet_res <- glmnet::glmnet(zz,y_fit,family = family,alpha=0)
    ...
    c(AUC,mean((data$ytest-family$linkinv(eta_hat))^2),mean((eta_hat-xtest%*%beta - alpha)^2),NA,NA,NA,NA,NA,check_gy)
  })
  ...
  resij[-c(7,8),-c(1:5)] <- apply(coefs,2, function(coef){
    ...
    if (max(abs(coef))<1e-12) {
      mar_coef <- c(coef(glm(y_fit~1,family = family)),rep(0,m))
    } else {
      glmnet_res <- glmnet::glmnet(zz,y_fit,family = family,alpha=0)
      mar_coef <- coef(glmnet_res,s=min(glmnet_res$lambda))
    }
  ...
  # ElNet 
  tmp_res <- myElNet(x,y_fit,xtest,family=family)
  ...
  # AdLASSO
  tmp_res <- myAdLASSO(x,y_fit,xtest,family=family)
  eta_hat <- xtest%*%tmp_res$beta + tmp_res$intercept
  if (family$family=="binomial" & weight_binom==1) {
    AUC <- my_AUC(data$ytest,tmp_res$yhat)
  } else {
  ...
  saveRDS(resij,paste0("../tmp_saved_files/ScrRPresi",i,"j",j,".rds"))
  resij
}

for (j in 1:nset) {
  for (i in 1:nrep) {
    res[i,,,j] <- readRDS(paste0("../tmp_saved_files/ScrRPresi",i,"j",j,".rds"))
  }
}

saveRDS(res,"../saved_results/Result_ScrRP_Experiment_25.rds")
